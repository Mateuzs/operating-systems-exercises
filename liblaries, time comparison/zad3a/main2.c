...
int main(int argc, const char * argv[]) {
    ...
    char *error;
    // Open the library
    handle = dlopen("libtableofblocks.so.1.0", RTLD_LAZY);
    if (!handle) {
        fprintf(stderr, "%s\n", dlerror());
        return 1;
    }
    ...
    deleteAndAddRecords = (void (*)(struct Table))dlsym(handle, "deleteAndAddRecords");
    ...
    deleteAndAddRecordsInStaticTable = (void (*)(struct StaticTable))dlsym(handle, "deleteAndAddRecordsInStaticTable");
